...
// iq3xxs_grid_c now lives in iq_tables_c.h
...
// Uses standard expf() matching llama.cpp's ggml_vec_soft_max_f32.
// Double-precision sum accumulation for numerical stability.
void vec_softmax(float* x, int n) {
    ...
    // exp(x[i] - max) and sum using standard expf, double accumulation
    double sum = 0.0;
    for (i = 0; i < n; i++) {
        float val = expf(x[i] - maxVal);
        x[i] = val;
        sum += (double)val;
    }
